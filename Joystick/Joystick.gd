...
func _gui_input(event: InputEvent) -> void:
	if (ui_mode == UI_Mode.MOVING or ui_mode == UI_Mode.FOLLOWING) and (event is InputEventScreenTouch and event.pressed):
...
func _update_output(vector: Vector2) -> void:
	...
	if vector_mode == Vector_Mode.REAL:
		output *= (vector.length() - dead_zone) / (clamp_zone - dead_zone)
